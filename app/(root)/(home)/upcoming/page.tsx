...
const Upcoming = () => {
  ...
  const { getUpcomingMeetings } = useApiClient();
  ...
  if (isLoading) {
    return <div className="text-white">Loading upcoming meetings...</div>;
  }
  ...
  return (
    <section className="flex flex-col gap-4 text-white">
      ...
      {meetings.length === 0 ? (
        <p className="text-sky-1">No upcoming meetings found.</p>
      ) : (
          ...
          {meetings.map((meeting) => (
            ...
            >
              ...
              <div className="flex flex-col items-end gap-2">
                <span className="rounded bg-blue-1 px-2 py-1 text-xs">
                  {meeting.status}
                </span>
                <Button
                  className="bg-blue-1"
